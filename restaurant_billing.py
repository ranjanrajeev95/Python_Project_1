...
lblTitle = Label(Tops, font=('arial', 30, 'bold'), text='Restaurant Billing System', bd=15, bg='white',
...
def TotalofUnit():
    ...
    DrinksPrices = "Rs." + str('%.2f' % Prices_Drinks)
    FoodsPrices = "Rs." + str('%.2f' % Prices_Food)
    Total_of_Food.set(FoodsPrices)
    Total_of_Drinks.set(DrinksPrices)
    SC = "Rs." + str('%.2f' % 1.59)
    ServiceCharge.set(SC)

    Sub_Total_of_Unit = "Rs." + str('%.2f'%(Prices_Drinks + Prices_Food + 1.59))
    SubTotal.set(Sub_Total_of_Unit)

    Tax = "Rs." + str('%.2f'%((Prices_Drinks + Prices_Food + 1.59) * 0.15))
    PaidTax.set(Tax)

    TT = ((Prices_Drinks + Prices_Food + 1.59) * 0.15)
    TC = "Rs." + str('%.2f'%(Prices_Drinks + Prices_Food + 1.59 + TT))
    TotalCost.set(TC)
